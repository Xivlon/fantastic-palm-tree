...
def adv_lookup(symbol): return 10_000_000
...
def test_trailing_not_active_before_activation_r():
    ...
    bar1 = Bar("ACT", base + datetime.timedelta(minutes=1), 102,102.5,101.8,102,12000)
    ctx.on_bar_start(bar1, broker)
    fills = ctx.process_symbol("ACT", bar1, broker, adv_lookup)
    broker.apply_fills(fills)

    bar2 = Bar("ACT", base + datetime.timedelta(minutes=2), 100,100.5,99.5,99.8,15000)
