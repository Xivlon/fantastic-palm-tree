    ...
    def remove_shares(self, quantity: int) -> float:
        ...
        if self.quantity > 0:
            ...
            realized_pnl = quantity * (
                0 - cost_per_share
            )  # Will be calculated with actual sell price
...
class Portfolio:
    ...
    def buy(self, symbol: str, quantity: int, price: float | None = None) -> Order:
        """Place a buy order."""
        order = Order(
            symbol=symbol,
            quantity=quantity,
    ...
    def sell(self, symbol: str, quantity: int, price: float | None = None) -> Order:
        """Place a sell order."""
        order = Order(
            symbol=symbol,
            quantity=quantity,
