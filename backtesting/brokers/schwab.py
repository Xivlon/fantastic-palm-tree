...
class SchwabBroker(BaseBroker):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if account_info:
                ...
                self.logger.info(
                    f"Connected to Schwab API for account {self.account_id}"
                )
                return True
            else:
                return False
                    ...
                    if await self.auth.refresh_access_token():
                        ...
                        async with self.session.request(
                            ...
                            if retry_response.status == 200:
                                ...
                                self.logger.error(
                                    f"API request failed after retry: {retry_response.status} - {error_text}"
                                )
                                return None
                    else:
            ...
            for pos_data in account_data.get("positions", []):
                ...
                position = Position(
                    ...
                    day_change_percent=float(
                        pos_data.get("currentDayProfitLossPercentage", 0)
                    ),
                )
                positions.append(position)

            # Parse account balances
            account_data.get("initialBalances", {})
    ...
    async def place_order(self, order: BrokerOrder) -> str | None:
        """Place an order."""
        ...
        if response is not None:
            ...
            order_id = (
                f"SCHWAB_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{order.symbol}"
            )
            return order_id
        return None
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    self.logger.error(
                        f"Quote request failed: {response.status} - {error_text}"
                    )
                    return None
    ...
    async def get_historical_data(
        ...
        params = {"symbol": symbol, "period": period, "frequency": interval}
        headers = self.auth.get_auth_headers()
        ...
        try:
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    self.logger.error(
                        f"Historical data request failed: {response.status} - {error_text}"
                    )
                    return []
