...
class KillSwitchTrigger(ABC):
    ...
    @abstractmethod
    def check(self, portfolio: Portfolio, current_prices: Dict[str, float], 
...
class VolatilityTrigger(KillSwitchTrigger):
    ...
    def __init__(self, max_volatility: float = 0.50, lookback_days: int = 30, 
    ...
    def check(self, portfolio: Portfolio, current_prices: Dict[str, float], 
        ...
        self.return_history = [(t, v, r) for t, v, r in self.return_history if t >= cutoff_date]
        ...
        if volatility >= self.max_volatility:
            reason = f"Volatility {volatility:.2%} exceeds limit {self.max_volatility:.2%}"
            self.activate(reason, timestamp)
            return True
...
class TimeBasedTrigger(KillSwitchTrigger):
    ...
    def __init__(self, start_time: str = "09:30", end_time: str = "16:00", 
                 trading_days_only: bool = True, name: str = "TimeBasedTrigger"):
        super().__init__(name)
...
class VaRTrigger(KillSwitchTrigger):
    ...
    def __init__(self, var_limit: float = 0.05, confidence: float = 0.95, 
                 lookback_days: int = 252, name: str = "VaRTrigger"):
        super().__init__(name)
