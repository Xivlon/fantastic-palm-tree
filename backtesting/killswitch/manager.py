...
class KillSwitchManager:
    ...
    def check_triggers(self, portfolio: Portfolio, current_prices: Dict[str, float], 
        ...
        for trigger in self.triggers:
            if trigger.check(portfolio, current_prices, timestamp):
                if trigger not in self.activated_triggers:
                    ...
                    self._on_trigger_activated(trigger, portfolio, current_prices, timestamp)
                    
        # Kill switch is active if any trigger is activated
        self.is_active = len(self.activated_triggers) > 0
        return self.is_active
        
    def _on_trigger_activated(self, trigger: KillSwitchTrigger, portfolio: Portfolio,
                             current_prices: Dict[str, float], timestamp: datetime) -> None:
...
def create_default_kill_switches(max_drawdown: float = 0.20, 
                                max_loss: float = 50000,
                                max_volatility: float = 0.50) -> KillSwitchManager:
