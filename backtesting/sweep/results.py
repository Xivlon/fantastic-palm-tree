...
class OptimizationResults:
    ...
    def get_worst_result(self) -> dict[str, Any] | None:
    ...
    def get_top_n_results(self, n: int = 10) -> list[dict[str, Any]]:
        ...
        sorted_results = sorted(
            self._successful_results, key=lambda x: x["objective_value"], reverse=True
        )
    ...
    def get_parameter_sensitivity(self) -> dict[str, dict[str, float]]:
        ...
        for param in param_columns:
            if param in df.columns:
                grouped = df.groupby(param)["objective_value"].agg(
                    ["mean", "std", "count"]
                )
                sensitivity[param] = {
    ...
    def plot_parameter_sweep(
        self, param1: str, param2: str = None, metric: str = "objective_value"
    ):
