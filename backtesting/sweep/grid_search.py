...
class GridSearchOptimizer(ParameterOptimizer):
    ...
    def optimize(self, data: pd.DataFrame, parameter_space: ParameterSpace,
                 verbose: bool = True) -> OptimizationResults:
...
class RandomSearchOptimizer(ParameterOptimizer):
    ...
    def optimize(self, data: pd.DataFrame, parameter_space: ParameterSpace,
                 n_iterations: int = 100, verbose: bool = True) -> OptimizationResults:
        ...
        if verbose:
            ...
            print(f"Trying {len(selected_combinations)} combinations out of {len(all_combinations)} possible")
            print(f"Using {self.max_workers or 'all available'} workers")
