...
def demonstrate_modular_architecture():
    ...
    try:
        ...
        if strategy.position:
            ...
            print(f"   Position: price={pos.entry_price:.2f}, size={pos.size}, atr={pos.entry_atr:.2f}")
    ...
    for i, (high, low, close, prev_close) in enumerate(trailing_bars):
        result: BarProcessResult = strategy.process_bar(high, low, close, prev_close)
        print(f"   Bar {i+5}: ATR={result.atr:.2f}, Stop Hit={result.stop_hit}, Stop Price={result.stop_price}")
        
        if result.exit_result:
            exit_res: ExitResult = result.exit_result
            print(f"   🎯 Position exited: PnL=${exit_res.pnl:.2f}, R-Multiple={exit_res.r_multiple:.2f}")
            print(f"      Reason: {exit_res.reason}, Commission: ${exit_res.commission:.2f}")
            break
